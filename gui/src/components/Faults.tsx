...
export default class Faults extends Component<FaultsProps, FaultsState>{
    ...
    render(){
        return(
            <Wrapper>
                <Container>
                    {Object.entries(getFaults()).map(([key, {state, description}], i) => {
                        return(
                            state && <FaultLabel faulty={true} key={i} title={description}>{key}</FaultLabel>
                        )
                    })}
                </Container>
            </Wrapper>
...
const FaultLabel = styled.div<{faulty: boolean}>`
    ...
    border: ${({faulty}) => faulty ? '1px solid #B0002060' : '1px solid #46b477A0'};
    &: hover{
        border: ${({faulty}) => faulty ? '1px solid #B00020' : '1px solid #46b477'}
    }
`